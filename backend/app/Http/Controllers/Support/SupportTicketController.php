...
class SupportTicketController extends Controller
{
    ...
    public function index(Request $r) { $p = $this->service->listForUser($r->user(), $r->only(['status', 'category', 'priority']), min(100, max(1, $r->integer('per_page', 20)))); return ResponseHelper::success(['tickets' => $p->items(), 'pagination' => ['current_page'=>$p->currentPage(),'last_page'=>$p->lastPage(),'per_page'=>$p->perPage(),'total'=>$p->total()]]); }
