...
export default function WishlistPage(){ const [items,setItems]=useState<WishlistItem[]>([]); const [loading,setLoading]=useState(true); const [error,setError]=useState('');
  const load=async(silent=false)=>{if(!silent)setLoading(true);try{setItems((await wishlistService.listAll()).items||[]);setError('');}catch{setError('Could not load saved items.');}finally{if(!silent)setLoading(false);}}; useEffect(()=>{void load();},[]); useLiveRefresh(()=>load(true),true,30000);
