...
export const supportService = {
  async list(params?: TicketFilters) { const response = await apiClient.get('/seller/support/tickets', { params }); return response.data.data as { tickets: SellerSupportTicket[]; pagination: TicketPagination }; },
  async create(data: TicketPayload) { const response = await apiClient.post('/seller/support/tickets', data, requestConfig(data)); return response.data.data.ticket as SellerSupportTicket; },
  async show(id: number) { const response = await apiClient.get(`/seller/support/tickets/${id}`); return response.data.data.ticket as SellerSupportTicket; },
  async reply(id: number, data: TicketPayload) { const response = await apiClient.post(`/seller/support/tickets/${id}/messages`, data, requestConfig(data)); return response.data.data.message as SupportMessage; },
  async close(id: number) { const response = await apiClient.post(`/seller/support/tickets/${id}/close`); return response.data.data.ticket as SellerSupportTicket; },
  async reopen(id: number) { const response = await apiClient.post(`/seller/support/tickets/${id}/reopen`); return response.data.data.ticket as SellerSupportTicket; },
};
