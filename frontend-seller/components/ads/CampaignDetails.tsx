...
export default function CampaignDetails({ id, admin = false }: { id: number; admin?: boolean }) {
  ...
  return <div className="space-y-6 text-slate-900">
    ...
    <div className="grid md:grid-cols-3 gap-4 bg-white border rounded-xl p-5">
      ...
      <div><p className="text-slate-500 text-sm">Campaign dates</p><p>{formatCampaignTime(c.starts_at)} → {formatCampaignTime(c.ends_at)}</p><p className="text-xs text-slate-500">Scheduled in {c.schedule_timezone || 'UTC'} · shown in your device timezone ({browserTimezone()}). Daily budgets reset at 00:00 UTC.</p></div>
      ...
      <div><p className="text-slate-500 text-sm">Payment source</p><p className="font-semibold">{c.funding_source === 'seller_wallet' ? 'Seller Wallet' : 'Legacy user wallet'}</p><p className="text-xs text-slate-500">{c.funding_source === 'seller_wallet' ? 'Reservation is held in Seller Wallet; billable clicks become platform advertising revenue.' : 'Historic campaign preserved on its original funding source.'}</p></div>
    </div>
    ...
    <div className="bg-white rounded-xl border p-5">
      ...
      {tab === 'transactions' && <div className="overflow-auto"><table className="w-full text-sm text-left"><thead><tr>{['Date', 'Type', 'Amount', 'Campaign reserve after', 'Seller Wallet', 'Destination'].map(h => <th key={h} className="pb-3 pr-4">{h}</th>)}</tr></thead><tbody>{transactions?.data.map(tx => <tr key={tx.id} className="border-t"><td className="py-3">{new Date(tx.created_at).toLocaleString()}</td><td className="capitalize">{label(tx.type)}</td><td>{money(tx.amount_cents)}</td><td>{money(tx.balance_after_cents)}</td><td className="text-xs">{tx.metadata?.wallet_balance_before_cents !== undefined && <>Before {money(Number(tx.metadata.wallet_balance_before_cents))}<br /></>} {tx.metadata?.wallet_balance_after_cents !== undefined && <>After {money(Number(tx.metadata.wallet_balance_after_cents))}</>} {tx.metadata?.wallet_ad_reserve_after_cents !== undefined && <>Ads reserve {money(Number(tx.metadata.wallet_ad_reserve_after_cents))}</>}</td><td className="text-xs">{String(tx.metadata?.funds_destination || tx.metadata?.funding_source || '—').replaceAll('_', ' ')}</td></tr>)}</tbody></table><div className="flex gap-4 pt-4"><button disabled={page <= 1} onClick={() => setPage(p => p - 1)}>Previous</button><span>Page {page} of {transactions?.last_page || 1}</span><button disabled={page >= (transactions?.last_page || 1)} onClick={() => setPage(p => p + 1)}>Next</button></div></div>}
