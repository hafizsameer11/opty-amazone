...
import { adService, adError, formatCampaignTime, money, label, type Campaign, type Page } from '@/services/ad-service';
...
export default function CampaignList({ admin = false, refreshKey = 0 }: { admin?: boolean; refreshKey?: number }) {
  ...
  const [rows, setRows] = useState<Page<Campaign> | null>(null); const [error, setError] = useState(''); const [loading, setLoading] = useState(true); const [deletingId, setDeletingId] = useState<number | null>(null);
  ...
  async function remove(campaign: Campaign) {
    if (!window.confirm(`Delete “${campaign.name}”? This stops delivery and releases any unused reserved budget.`)) return;
    setDeletingId(campaign.id); setError('');
    try { await adService.destroy(campaign.id); await load(); } catch (e) { setError(adError(e)); } finally { setDeletingId(null); }
  }
  ...
  return <div className="space-y-4">
    ...
    <div className="overflow-auto rounded-xl border bg-white"><table className="w-full text-sm text-left"><thead className="bg-slate-50 text-slate-500"><tr>{['Campaign / product', 'Status / payment', 'Dates', 'Budget / remaining', 'Spend', 'Impressions', 'Clicks / CTR', 'CPC', 'Conversions / revenue', 'ROAS', 'Actions'].map(h => <th className="p-3 whitespace-nowrap" key={h}>{h}</th>)}</tr></thead>
